...
log_file= "vm_log.txt"
port = 22
command_lst = ["sudo mv /etc/nginx/nginx.conf /etc/nginx/nginx_old.conf" ,"sudo ls /etc/nginx", "sudo cp nginx.conf /etc/nginx/nginx.conf", "cat /etc/nginx/nginx.conf", "sudo systemctl restart nginx"]
...
autoscale_template= "nap-autoscale-ubuntu-dev.js"
...
del_vmss_lb= "az network lb delete --name " + vmssName + "LB" + "  --resource-group " + resource_group
del_dashboard="az portal dashboard delete --name " + db_name + "  --resource-group " + resource_group + " -y "
del_vmss_ip= " az network public-ip delete -g " + resource_group + " -n " + "autoscale-Demo-ip"
